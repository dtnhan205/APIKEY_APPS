function app() {
  return {
    ...
    async createPackage() {
      ...
      try {
        const res = await fetch('http://localhost:3000/api/packages/create', {
          method: 'POST', 
          headers: {
    ...
    async createKey() {
      ...
      try {
        const res = await fetch('http://localhost:3000/api/keys/create', {
          method: 'POST', 
          headers: {
            'Content-Type':'application/json', 
            'Authorization': `Bearer ${this.token}`
    ...
    async loadKeys() {
      try {
        const res = await fetch('http://localhost:3000/api/keys/list', {
          headers: {'Authorization': `Bearer ${this.token}`}
        });
    ...
    async resetKey(id) {
      ...
      try {
        const res = await fetch(`http://localhost:3000/api/keys/${id}/reset`, {
          method: 'POST', 
          headers: {'Authorization': `Bearer ${this.token}`}
        });
